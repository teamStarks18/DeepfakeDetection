...
def combine_frames_to_video(input_path, output_path):
    ...
    for filename in os.listdir(input_path):
        if filename.endswith('.jpg'):
            video_name, frame_number = filename.split('_')[0], filename.split('_')[1]
            video_files[(video_name, frame_number)].append(filename)

    for video_info, frames in video_files.items():
        # Sort frames based on their sequence number
        frames.sort(key=lambda x: int(x.split('_')[-1].split('.')[0]))

        # Create video writer
        output_filename = f'{"_".join(video_info)}.mp4'
